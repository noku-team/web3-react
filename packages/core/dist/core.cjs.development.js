...
function useWeb3ReactManager() {
  ...
  var changeAccountIndex = React.useCallback(function (accountIndex) {
    try {
      if (!connector) {
        throw new Error("Connector doesn't exist yet");
      }

      return Promise.resolve(connector.getAccount(accountIndex)).then(function (accountAtIndex) {
        if (!accountAtIndex) {
          throw new Error('Something went wrong when getting account index: ' + accountIndex);
        }

        var account = normalizeAccount(accountAtIndex);
        dispatch({
          type: ActionType.UPDATE,
          payload: {
            account: account
  ...
  }, [connector]); // ensure that connectors which were set are deactivated
...
function createWeb3ReactRoot(key) {
  ...
  CONTEXTS[key] = React.createContext({
    ...
    changeAccountIndex: function changeAccountIndex() {
        invariant(false, 'No <Web3ReactProvider ... /> found.')  ;
    },
    active: false
  });
  ...
  return function Web3ReactProvider(_ref) {
    ...
    var _useWeb3ReactManager = useWeb3ReactManager(),
        ...
        changeAccountIndex = _useWeb3ReactManager.changeAccountIndex;
